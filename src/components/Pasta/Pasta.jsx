...
function Pasta(props) {
  ...
  const pasta = menu.pasta;

  React.useEffect(() => {
    if (props.screenWidth >= 1280) {
      setRenderCards(pasta.slice(0, 12));
    }
    if (props.screenWidth <= 1090) {
      setRenderCards(pasta.slice(0, 6));
    }
    if (props.screenWidth <= 425) {
      setRenderCards(pasta.slice(0, 4));
    }
    if (renderCards.length === pasta.length) {
      setShowMoreButton(false);
    } else {
      setShowMoreButton(true);
    }
  }, [pasta, props.screenWidth]);
  ...
  React.useEffect(() => {
    ...
    if (props.screenWidth >= 1280) {
      pasta.length > 12 ? setShowMoreButton(true) : setShowMoreButton(false);
    }
    if (props.screenWidth <= 1090) {
      pasta.length > 6 ? setShowMoreButton(true) : setShowMoreButton(false);
    }
    if (props.screenWidth <= 425) {
      pasta.length > 5 ? setShowMoreButton(true) : setShowMoreButton(false);
    }
  }, [props.screenWidth, pasta.length]);

  function handleShowFilms() {
    if (props.screenWidth >= 1280) {
      setRenderCards(pasta.slice(0, renderCards.length + 3));
      if (renderCards.length >= pasta.length - 3) {
        setShowMoreButton(false);
      }
    }
    if (props.screenWidth <= 1090) {
      setRenderCards(pasta.slice(0, renderCards.length + 2));
      if (renderCards.length >= pasta.length - 2) {
        setShowMoreButton(false);
      }
    }
    if (props.screenWidth <= 425) {
